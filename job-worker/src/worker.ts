...
export class JobWorker {
  ...
  constructor() {
    this.workerId = `worker-${process.env.RAILWAY_REPLICA_ID || uuidv4().slice(0, 8)}`;
   ...
   */
  private async executeViaVercelAPI(job: Job): Promise<TaskResult> {
    ...
    const commandData = (await commandResponse.json()) as { commandId: string };
  ...
  private async pollForCompletion(
    ...
    while (attempts < maxAttempts && !this.isShuttingDown) {
      ...
      const data = (await response.json()) as { commands?: Array<{ id: string; status: string; result?: TaskResult; error_message?: string }> };
   ...
   */
  private async syncProgressFromCommand(
    jobId: string,
    command: { result?: TaskResult; status: string }
  ): Promise<void> {
    if (!command.result?.steps?.length) return;
    ...
    if (currentStepData) {
      ...
      const progress = calculateOverallProgress(step, currentStepData.progress || 0);

      await this.updateProgress(
        jobId,
        progress,
        step,
        currentStepData.message || `Processing ${step}...`
      );
    }
  }
   ...
   */
  private mapStepName(step: string): JobStep {
    const mapping: Record<string, JobStep> = {
      scraping: "scraping",
      scrape: "scraping",
   ...
   * Update job with command ID
   */
  private async updateJobCommandId(
    jobId: string,
    commandId: string
  ): Promise<void> {
    const supabase = getSupabase();
   ...
   * Mark job as completed
   */
  private async completeJob(jobId: string, result: TaskResult): Promise<void> {
    const supabase = getSupabase();
